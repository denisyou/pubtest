...
function sfile_distinct( sfile ) {
      ...
      sfile.forEach(function(d) {
        if ( res.some(function(dd) { return dd.k1 == d.k2 && dd.k2 == d.k1 && d.v == dd.v; }) == false ) {
          res.push(d);
        }
      });
      return res;
...
// 將json格式導入ForceSimulation
 function jsonToForceSim( jsonData ) {
      ...
      jsonData.forEach(function(d, i) {
        ...
        data.link.push( { source: buf.indexOf(d.k1), target: buf.indexOf(d.k2), rel: d.v, linkID: i } );
      }); 
      return data;
   }
// ObjArray 系列
...
function forceRefresh() {
    ...
    node = svgZoomed.selectAll("circle")
            ...
            .on('mouseover', function(d) { 
              d3.select(this).select('circle').transition().attr('r', d.freq * forceProps.collideMag + forceProps.collideBase );
    ...
    node.append('circle').attr("r", function(d){ return d.freq * forceProps.collideMag + forceProps.collideBase })
   ...
   simulation.nodes(data.name).on("tick", ticked);
    ...
    // 階層縮放
    if ( forceProps.distFilter <= 7 && forceProps.distFilter > 4 ) {
      svgZoomed.transition().call( zoom.scaleTo, 0.35 );
    } else if ( forceProps.distFilter <= 4 ) {
      svgZoomed.transition().call( zoom.scaleTo, 0.5 );
    } else if ( forceProps.freqLevel != 10 && forceProps.distFilter > 7 ) {
      svgZoomed.transition().call( zoom.scaleTo, 0.27 );
    
    }

    if ( forceProps.freqFilter <= 9 && forceProps.freqFilter > 6 ) {
      svgZoomed.transition().call( zoom.scaleTo, 0.4 );
    } else if ( forceProps.freqFilter <= 6 ) {
      svgZoomed.transition().call( zoom.scaleTo, 0.5 );
    } else if ( forceProps.distFilter != 10 && forceProps.freqLevel > 9) {
      svgZoomed.transition().call( zoom.scaleTo, 0.27 );
...
function forceReset( f ) {
  ...
  f.force("link", d3.forceLink().id(function(d) { return d.index }).distance(function(d){return forceProps.linkDist * tScale(d.rel);}).strength(forceProps.linkStr))
        ...
        .force("collide",d3.forceCollide().iterations(5).radius(function(d){ return (d.freq * forceProps.collideMag + forceProps.collideBase) }) )//d.freq * forceProps.collideMag + forceProps.collideBase
